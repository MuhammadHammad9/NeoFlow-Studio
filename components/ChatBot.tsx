                ...
                <div className="prose prose-sm md:prose-base max-w-none break-words">
                   <ReactMarkdown 
                    components={{
                       ...
                       strong: ({node: _node, ...props}) => <strong className="font-bold inherit" {...props} />,
                       a: ({node: _node, ...props}) => <a className="underline hover:opacity-80 inherit decoration-2 underline-offset-2" target="_blank" rel="noopener noreferrer" {...props} />,
                       code: ({node: _node, ...props}) => <code className={`font-mono text-sm px-1.5 py-0.5 rounded ${msg.role === 'user' ? `bg-${theme}-700/50` : (mode === 'dark' ? 'bg-black/30' : 'bg-slate-100')}`} {...props} />,
                       pre: ({node: _node, ...props}) => <pre className={`p-3 rounded-xl my-2 overflow-x-auto ${msg.role === 'user' ? `bg-${theme}-800/50` : (mode === 'dark' ? 'bg-black/40' : 'bg-slate-100')}`} {...props} />
                    }}
                   >
                    {msg.text}
                   </ReactMarkdown>
